...
import { getMinDataApi, runMinVariantApi } from '#/api/min';
import { getJobsApi } from '#/api/watch';

import HitTable from '../watch/HitTable.vue';

// 后端 30f 的 job key 统一带 "m30:" 前缀
const JOB_PREFIX = 'm30:';
const jobKey = (secKey: string) => `${JOB_PREFIX}${secKey}`;
...
let timer: any = null;
// 我主动发起、等待刷新的任务 key(已带 m30: 前缀)。捕捉秒退的变体，
// 仅靠"目击 running→done"会漏刷新。
...
// section.key 对应的后台 job 需按 m30:<key> 去找
function jobOf(secKey: string): undefined | WatchJob {
  return jobs.value[jobKey(secKey)];
}

async function load() {
  try {
    data.value = await getMinDataApi();
...
async function pollJobs() {
  try {
    ...
    for (const j of list) {
      // 只关心 30f 的任务
      if (!j.key.startsWith(JOB_PREFIX)) continue;
      next[j.key] = j;
...
async function runOne(secKey: string) {
  pending.add(jobKey(secKey));
  await runMinVariantApi(secKey);
  ElMessage.info('已开始，进度见下方');
  await pollJobs();
}

async function runAll() {
  for (const s of data.value?.sections ?? []) pending.add(jobKey(s.key));
  await runMinVariantApi('all');
