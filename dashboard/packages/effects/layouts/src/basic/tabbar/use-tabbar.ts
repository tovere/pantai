...
export function useTabbar() {
  ...
  const isTabRouteValid = (tab: TabDefinition) => {
    ...
    return !!lastMatched && lastMatched.name !== 'FallbackNotFound';
  };

  const pruneStaleTabs = (tabs: TabDefinition[]) => {
    const validTabs = tabs.filter((tab) => isTabRouteValid(tab));
    if (validTabs.length === tabs.length) {
      return validTabs;
    }

    tabbarStore.tabs = validTabs;
    tabbarStore.visitHistory.retain(
      validTabs.map((tab) => tab.key ?? getTabKey(tab)),
    );
  ...
  watch(
    ...
    ([tabs]) => {
      const validTabs = pruneStaleTabs(tabs);
      currentTabs.value = validTabs.map((item) => wrapperTabLocale(item));
    },
  );
