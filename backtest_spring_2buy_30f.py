#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""30 分钟级别 策略七(Spring + 二买底背驰)回测。

口径同日线版 backtest_spring_2buy.py, 但在 30 分序列上跑:
  - 信号: 复用 screen_spring_2buy_30f.check(判某根30分bar是否 Spring/二买入场点);
  - 大级别过滤: 日线趋势按**信号那根bar的日期之前的日线收盘**切片(不吃未来);
  - 出场: 止损贴 Spring 低点下方(极紧) + 启动后移动止损让利润奔跑, MAX_HOLD 根30分bar封顶;
  - 胜率 = ret>0 的笔数占比。
...
import screen_spring_2buy_30f as s30
...
def run_one(code, name, secid):
    ...
    while i < n - 1:
        ...
        rec = {"entry": entry, "spring_low": item["spring_low"]}
...
def main():
    ...
    print(f"{pool}池 {len(uni)} 只, 30分级别 策略七(Spring/二买+背驰) 回测中 "
